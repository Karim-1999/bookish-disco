...
if (isset($_POST['title'])) {
    ...
    if ($stm = $connect->prepare('UPDATE posts set  title = ?, content = ? , date = ?, `Url-image` = ?   WHERE id = ?')) {
        $stm->bind_param('ssssi', $_POST['title'], $_POST['content'], $_POST['date'],$_POST['Url-image'], $_GET['id']);
        $stm->execute();
